...
<div class="container">
                    <h4>แสดงชื่อหนัง</h4>
                    <a href="data5_detall_add.php" class="btn btn-link">เพิ่มชื่อหนัง</a>
                    <div class="container">
                    ...
                    <?php
                        include 'connect.php';                        
                        $sql =  'SELECT * FROM data5_detall order by day_id';
                        ...
                        while (($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) != NULL) {
                            ...
                            echo '<td>' . $row['day_day'] . '</td>';
                            echo '<td>' . $row['day_mouth'] . '</td>';
                            echo '<td>' . $row['day_year'] . '</td>';
                            echo '<td>' . $row['money_mn'] . '</td>';
                            ...
                            echo '<td>';
                            ?>
                                <a href="data5_detall_edit.php?day_id=<?php echo $row['day_id'];?>" class="btn btn-warning">แก้ไข</a>
                                <a href="JavaScript:if(confirm('ยืนยันการลบ')==true)
                                   {window.location='data5_detall_delete.php?day_id=<?php echo $row["day_id"];?>'}" class="btn btn-primary">ลบ</a>
                            <?php
                                    echo '</td>';                            
                            echo '</tr>';
                        }
